...
def test_adjudication_uses_stable_strict_eq_consensus() -> None:
    source = CONTRACT.read_text(encoding="utf-8")

    assert "gl.eq_principle.strict_eq(" in source
    assert "gl.nondet.web.get" in source
    assert "gl.nondet.exec_prompt" not in source
    assert "isinstance(leader_result, gl.vm.Return)" not in source
